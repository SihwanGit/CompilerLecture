...
public class Sint {
    ...
    State Eval(Assignment a, State state) {
        ...
        if (a.ar == null)
            ...
            Value arrayValue = (Value) state.get(a.ar.id);  // 배열을 가져옴
            Value[] array = arrayValue.arrValue();  // 배열을 Value[] 형태로 가져옴
            Value indexValue = V(a.ar.expr, state);  // 인덱스를 계산
            int index = indexValue.intValue();  // 인덱스를 정수로 변환
            
            // 인덱스 범위 체크 (배열의 크기를 벗어나지 않도록)
            check(index >= 0 && index < array.length, "Array index out of bounds");
            array[index] = v; // 배열의 해당 인덱스에 값 할당
            return state;
        }
    }
        ...
        if (ds != null) {
        	for (Decl decl : ds) 
			    if (decl.arraysize > 0) { // <type> id[n];
	                ...
	                // 배열 선언 처리
	                Value[] array = new Value[decl.arraysize];  // 새로운 배열 생성
	                for (int i = 0; i < decl.arraysize; i++) {
	                    array[i] = new Value(decl.type);  // 배열의 각 요소를 기본값으로 초기화
	                }
	                state.push(decl.id, new Value(array));  // 배열을 상태에 추가
	            }
    ...
    Value V(Expr e, State state) {
        ...
        if (e instanceof Array) { // id[<expr>]
            ...
            Array ar = (Array) e;  // Array 객체로 캐스팅
            Value arrayValue = (Value) state.get(ar.id);  // 배열 이름으로 배열을 가져옴
            Value[] array = arrayValue.arrValue();  // 배열로 변환
            Value indexValue = V(ar.expr, state);  // 인덱스를 계산
            int index = indexValue.intValue();  // 인덱스를 정수로 변환
            // 배열 인덱스가 유효한지 체크
            check(index >= 0 && index < array.length, "Array index out of bounds");
            return array[index];  // 배열 요소 반환        
	    }
